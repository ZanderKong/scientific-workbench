...
实验记录导入的暂态与 compact 记录位于 `registry/imports/<importId>.json`，其状态、CAS 与最小化约束见 [DATA_PROTOCOL.md](./DATA_PROTOCOL.md) 的“实验记录导入”一节。该记录不是可重建索引的一部分，也必须在无 SQLite 时从 registry 读取。
