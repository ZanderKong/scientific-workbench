...
项目面向个人科研工作流，提供浏览器界面、文件存储以及 REST / MCP 接口。当前为开发预览。AI 图片导入在「OpenCode 1.18.31 + `deepseek/deepseek-v4-flash-vision-exp` + 受限 profile」这一已真实验证的组合下可用（入口：样品 → ＋新建样品 右侧箭头）；换模型或换版本需要重新取证，其他限制见[当前状态](docs/STATUS.md)。
